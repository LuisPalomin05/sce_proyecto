...
const CrearConsulta = ({ titulo }) => {
  const localhost = `https://backendapi-6thn.onrender.com/api/${titulo}`;
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    try {
      const nuevaFactura = { ...formData };
      console.log("Formulario conectado");

      axios.post(localhost, nuevaFactura).then((response) => {
        console.log("Cotización creada:", response.data);
        Notify(
          `Registro de ${titulo} creada con éxito: ${response.data.rucCliente}`
        );
      });
    } catch (error) {
